...
//
//  Created by Muthulingam on 27/08/23.
//
...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader(){
        ...
        XCTAssertEqual(loader.loadFeedCallCount,2, "Expected another loading requests once user initates a  load")
        
        sut.simulateUserInitiatedFeedReload()
        XCTAssertEqual(loader.loadFeedCallCount,3, "Expected a third loading requests once user initates another  load")
    }
    ...
    func test_loadingFeedIndicator_isVisibleWhileLoadingFeed(){
        ...
        XCTAssertFalse(sut.isShowingLoadingIndicator, "Expected no loading indicator once user initiated a loading complete with error")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage(){
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator,false, "Expected no loading indicator for the first view once image loading completes successfully")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator,true, "Expected no loading indicator for the second view once first image loading completes successfully")
    ...
    func test_feedImageRetryButton_isVisibleOnInvalidImageData() {
        ...
        let view0 = sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expected no retry action for view while loading image")
        
        let invalidData = Data("invalid image data".utf8)
        loader.completeImageLoading(with: invalidData, at: 0)
        XCTAssertEqual(view0?.isShowingRetryAction, true, "Expected retry action for view while loading image completed with invalid image data")
    ...
    func test_feedImageView_preloadsImageURLWhenNearVisible() {
        ...
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [image0, image1])
        XCTAssertEqual(loader.loadedImageURLs, [], "Expected no image URLs requests until image is near visible")
    ...
    func test_feedImageView_cancelPreloadsImageURLWhenNotNearVisibleAnymore() {
        ...
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [image0, image1])
        XCTAssertEqual(loader.cancelledImageURLs, [], "Expected no cancelled image URLs requests until image is not near visible")
        
        sut.simulateFeedImageViewNotNearVisible(at: 0)
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url], "Expected first cancelled imageURL request once first image is not near visible")
    ...
    func test_loadFeedCompletion_dispatchesFromBackgrondToMainThread(){
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
    ...
    private func assertThat(_ sut: FeedViewController,isRendering feed:[FeedImage],file: StaticString = #file, line: UInt = #line){
        ...
        guard sut.numberOfRederedFeedImageViews() == feed.count else {
            return XCTFail("Expected \(feed.count) instance, get \(sut.numberOfRederedFeedImageViews()) instead",file: file,line: line)
        }
    ...
    private func assertThat(_ sut: FeedViewController, hasViewConfiguredFor image: FeedImage, at index: Int,file: StaticString = #file, line: UInt = #line ){
        ...
        XCTAssertEqual(cell.locationText, image.location,"Expected location text to be \(String(describing: image.location)) for imageview at index (\(index)) ",file: file,line: line)
        XCTAssertEqual(cell.descriptionText, image.description,"Expected location text to be \(String(describing: image.description)) for imageview at index (\(index)) ",file: file,line: line)
